...
TRANSLATIONS = {
    'ms': {
        ...
        'halal_compliance': 'Kepatuhan Halal',
...
def index():
    ...
    # Pass stats to index
    latest_gigs = Gig.query.filter_by(status='open').order_by(Gig.created_at.desc()).limit(5).all()
...
def register_page():
    ...
    if 'user_id' in session:
        return redirect('/dashboard')
    return render_template('register.html', lang=get_user_language(), t=t)
